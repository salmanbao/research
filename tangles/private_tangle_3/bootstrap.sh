...
# Create snapshot
mkdir -p snapshots/hornet-1c
sudo chmod -R 777 ./snapshots
docker compose run create-snapshots

# Prepare database directory for hornet-1c
mkdir -p privatedb/hornet-1c
mkdir -p privatedb/state
sudo chmod -R 777 ./privatedb


# Bootstrap network (create hornet-1c database, create genesis milestone, create coo state)
docker compose run bootstrap-network

# Duplicate snapshot for all nodes
cp -R snapshots/hornet-1c snapshots/hornet-2c
cp -R snapshots/hornet-1c snapshots/hornet-3c
cp -R snapshots/hornet-1c snapshots/hornet-4c
sudo chmod -R 777 ./snapshots
...
mkdir -p privatedb/hornet-2c
mkdir -p privatedb/hornet-3c
mkdir -p privatedb/hornet-4c
sudo chmod -R 777 ./privatedb
