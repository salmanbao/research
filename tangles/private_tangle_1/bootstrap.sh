...
# Create snapshot
mkdir -p snapshots/hornet-1a
sudo chmod -R 777 ./snapshots
docker compose run create-snapshots

# Prepare database directory for hornet-1a
mkdir -p privatedb/hornet-1a
mkdir -p privatedb/state
sudo chmod -R 777 ./privatedb


# Bootstrap network (create hornet-1a database, create genesis milestone, create coo state)
docker compose run bootstrap-network

# Duplicate snapshot for all nodes
cp -R snapshots/hornet-1a snapshots/hornet-2a
cp -R snapshots/hornet-1a snapshots/hornet-3a
cp -R snapshots/hornet-1a snapshots/hornet-4a
sudo chmod -R 777 ./snapshots
...
mkdir -p privatedb/hornet-2a
mkdir -p privatedb/hornet-3a
mkdir -p privatedb/hornet-4a
sudo chmod -R 777 ./privatedb
